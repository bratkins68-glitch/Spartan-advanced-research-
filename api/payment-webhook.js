...
export default async function handler(req, res) {
  ...
  try {
    ...
    const orderTo = process.env.ORDER_TO_EMAIL || process.env.ORDER_EMAIL;
    const orderFrom = process.env.ORDER_FROM_EMAIL || "Spartan Orders <onboarding@resend.dev>";

    if (!ipnSecret) {
      return res.status(500).json({ error: "NOWPAYMENTS_IPN_SECRET is missing" });
    }
    if (!resendApiKey) {
      return res.status(500).json({ error: "RESEND_API_KEY is missing" });
    }
    if (!orderTo) {
      return res.status(500).json({ error: "ORDER_TO_EMAIL (or ORDER_EMAIL) is missing" });
    }
    ...
    if (paymentStatus !== "finished") {
      return res.status(200).json({ received: true, payment_status: paymentStatus });
    }

    const orderId = body.order_id || "Unknown";
    const paymentId = body.payment_id || "Unknown";
    const description = body.order_description || "No order description";
    ...
    const html = `
      ...
      <p><strong>Order Total:</strong> ${priceAmount} ${priceCurrency}</p>
      <p><strong>Crypto Paid:</strong> ${actuallyPaid} ${payCurrency}</p>
      <hr>
    ...
    if (!emailResponse.ok) {
      ...
      return res.status(500).json({ error: "Payment confirmed but email failed", details: emailResult });
    }

    return res.status(200).json({ received: true, emailed: true, order_id: orderId });
  } catch (err) {
    console.error("Webhook error:", err);
    return res.status(500).json({ error: err instanceof Error ? err.message : "Webhook error" });
  }
}
